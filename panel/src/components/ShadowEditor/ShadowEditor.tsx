...
export function ShadowEditor({
  ...
  function allClassesForLayer(layer: ShadowLayerState): string[] {
    ...
    if (layer.colorClass) {
      classes.push(layer.opacity !== null ? `${layer.colorClass}/${layer.opacity}` : layer.colorClass);
    }
    return classes;
  }
  ...
  return (
    /**
     * 7-column shared grid — every row (active + ghost) places cells directly here.
     * Col 1: dot (●) or [+] button  — 16px fixed
     * Col 2: layer name             — auto (widest = "Inset Shadow" sets all)
     * Col 3: size scrubber          — 1fr
     * Col 4: color swatch           — 26px
     * Col 5: opacity scrubber       — 1fr
     * Col 6: preview square         — 26px
     * Col 7: × remove button        — 18px
     */
    <div
      className="grid items-center gap-x-1.5 gap-y-1"
      style={{ gridTemplateColumns: '16px auto 1fr 26px 1fr 26px 18px' }}
    >
          ...
          {layer ? (
            <ShadowLayerRow
              ...
              onOpacityHover={(value) => {
                ...
                const oldClass = layer.opacity !== null ? `${base}/${layer.opacity}` : base;
