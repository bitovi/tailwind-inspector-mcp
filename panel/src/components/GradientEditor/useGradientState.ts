...
export function useGradientState(props: GradientEditorProps) {
  ...
  const [solidColorName, setSolidColorName] = useState<string | null>(initialSolidName);
  const [solidColorHex, setSolidColorHex] = useState<string | null>(initialSolidHex);
  const [selectedStopId, setSelectedStopId] = useState<string | null>(null);
  const [colorPickerOpen, setColorPickerOpen] = useState(false);
  const [editingTarget, setEditingTarget] = useState<'stop' | 'solid' | null>(null);
  ...
  const handleDirectionClick = useCallback((dir: GradientDirection) => {
    setDirection(dir);
    setMode('gradient');
    ...
    if (mode === 'gradient' && stops.length >= 2) {
      ...
      // Stage the direction + both color stops; remove any existing bg- solid class
      const oldBg = solidColorName ? `bg-${solidColorName}` : '';
      onStage(oldBg, `bg-gradient-to-${dir}`);
      onStage('', `from-${baseColor}`);
      onStage('', `to-${baseColor}`);
    }
  }, [direction, mode, stops, solidColorName, solidColorHex, props.colors, onStage]);
  ...
  // --- Stop dragging ---
  const handleStopDrag = useCallback((stopId: string, newPosition: number) => {
    setStops((prev) =>
      prev.map((s) => (s.id === stopId ? { ...s, position: newPosition } : s))
    );
    const stop = stops.find((s) => s.id === stopId);
    if (stop) {
      const oldPos = stop.position != null ? `${stop.role}-${stop.position}%` : '';
      const newPos = `${stop.role}-${newPosition}%`;
      onPreview(oldPos, newPos);
    }
  }, [stops, onPreview]);

  const handleStopDragEnd = useCallback((stopId: string, position: number) => {
    setStops((prev) =>
  ...
  const handleStopClick = useCallback((stopId: string, el: Element) => {
  ...
  // --- Remove stop ---
  const handleStopRemove = useCallback((stopId: string) => {
    const stop = stops.find((s) => s.id === stopId);
    if (!stop) return;
    ...
    if (remaining.length >= 2) {
      ...
      stops.forEach((s) => onStage(`${s.role}-${s.colorName}`, ''));
  ...
  const handleColorHover = useCallback((fullClass: string) => {
  ...
  const handleColorClick = useCallback((fullClass: string) => {
    ...
    if (editingTarget === 'stop' && selectedStopId) {
      ...
      if (stop) {
        ...
        const colorName = fullClass.startsWith(prefix) ? fullClass.slice(prefix.length) : fullClass;
        setStops((prev) =>
          prev.map((s) =>
            s.id === selectedStopId
              ? { ...s, colorName, hex: resolveColorHex(colorName, props.colors) }
              : s
          )
        );
        onStage(`${stop.role}-${stop.colorName}`, `${stop.role}-${colorName}`);
        setColorPickerOpen(false);
        setSelectedStopId(null);
        setEditingTarget(null);
        setAnchorEl(null);
  ...
  }, [editingTarget, selectedStopId, stops, solidColorName, onStage, props.colors]);
  ...
  const colorPrefix = editingTarget === 'stop' && selectedStopId
    ? (stops.find((s) => s.id === selectedStopId)?.role ?? 'from') + '-'
    : 'bg-';

  const colorPickerCurrentValue = editingTarget === 'stop' && selectedStopId
