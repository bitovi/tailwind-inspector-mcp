    ...
    <div>
      <div className={`flex items-center gap-2 px-3 py-1 ${isEdited ? 'bg-bit-orange/5' : ''}`}>
        ...
        <input
          ...
          className="w-28 bg-transparent text-[10px] text-bit-text font-mono border border-bit-border rounded px-1.5 py-0.5 focus:border-bit-teal focus:outline-none"
          spellCheck={false}
        />
        {pxPer !== null && (
          <span className="text-[9px] text-bit-muted">
            1 unit = {pxPer}px
          </span>
        )}
      </div>
        ...
        <div className="flex items-end gap-3 px-3 py-2">
          {PREVIEW_STEPS.map(n => (
            <div key={n} className="flex flex-col items-center gap-1">
              <div
                className="bg-bit-teal/30 rounded-sm"
                style={{ width: Math.min(pxPer * n, 80), height: 8 }}
                title={`${n} × ${displayValue} = ${pxPer * n}px`}
              />
              <span className="text-[8px] text-bit-muted">{n}</span>
            </div>
          ))}
        </div>
      )}
