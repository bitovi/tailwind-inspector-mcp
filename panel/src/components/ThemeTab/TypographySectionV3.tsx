...
interface TypographySectionV3Props {
...
/** v3 fontSize values can be a string ("1rem") or a tuple ["1rem", { lineHeight: "1.5rem" }] */
function parseFontSize(raw: unknown): { size: string; lineHeight: string | undefined } {
  if (typeof raw === 'string') return { size: raw, lineHeight: undefined };
  if (Array.isArray(raw)) {
    const size = String(raw[0]);
    const opts = raw[1];
    const lineHeight = typeof opts === 'object' && opts !== null && 'lineHeight' in opts
      ? String((opts as Record<string, unknown>).lineHeight)
      : undefined;
    return { size, lineHeight };
  }
  return { size: String(raw), lineHeight: undefined };
}
  ...
  return (
    <div className={`flex items-center gap-2 px-3 py-1 ${isEdited ? 'bg-bv-orange/5' : ''}`}>
      ...
      <input
        ...
        value={displaySize}
        onChange={(e) => {
          onEdit(`fontSize-${name}`, {
            variable: `fontSize.${name}`,
            value: e.target.value,
            lineHeight: displayLH || undefined,
          });
        }}
      ...
      {lineHeightValue !== undefined && (
        <>
          ...
          <input
            ...
            spellCheck={false}
            title={`theme.fontSize.${name}[1].lineHeight`}
...
export function TypographySectionV3({ fontSize, fontWeight, edits, onEdit }: TypographySectionV3Props) {
  const sizeKeys = [
    ...
    <div>
      {sizeKeys.length > 0 && (
        <div>
          ...
          {sizeKeys.map(key => {
            const parsed = parseFontSize(fontSize[key]);
            return (
              <FontSizeRow
                key={key}
                name={key}
                sizeValue={parsed.size}
                lineHeightValue={parsed.lineHeight}
                editedSize={edits.get(`fontSize-${key}`)?.value}
                editedLineHeight={edits.get(`fontSizeLH-${key}`)?.lineHeight}
                onEdit={onEdit}
              />
            );
          })}
